...
export class UserConfirmOrderComponent implements OnInit {
  ...
  prodList:any;
  constructor(private userService:SharedService, private activatedRoute: ActivatedRoute,
  ...
  get a(){ return this.orderForm.get('address');}
  get e(){return this.orderForm.get('endDate');}
  get s(){return this.orderForm.get('startDate');}
...
this.currentOrder.cost=Math.ceil((difftime/(1000*60*60)))*this.prodList[this.index].perHourCost;
...
this.currentOrder.prodName=this.routeParams.id2;
    this.userService.postOrderData(this.currentOrder).subscribe((data) => {
      alert('Order Confirmed! Please pay Rs. '+this.currentOrder.cost+' to the CyloWala.');
    });
