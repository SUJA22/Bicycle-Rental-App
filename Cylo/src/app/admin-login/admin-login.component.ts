...
export class AdminLoginComponent implements OnInit {
  ...
  get e(){ return this.adminLoginForm.get('username');}
  get p(){return this.adminLoginForm.get('password');}
  ngOnInit(): void {
  }
addAdmin(){
  ...
  for (let i = 0; i < this.adminList.length; i++) {
    if(this.adminLoginForm.value.username==this.adminList[i].username && this.adminLoginForm.value.password==this.adminList[i].password){
  flag=true;
  index=i;
  break;
    }
